...
export function toDynamoItem(userId, deadline, deadlineId) {
  return {
    ...
    // GSI3 keys — only include when there's an actual reminder scheduled
    ...(deadline.reminderSchedule?.[0] ? { nextReminderAt: deadline.reminderSchedule[0], reminderStatus: 'PENDING' } : {}),
    remindersDismissed: 0,
    paceModeSessionsCompleted: 0,
    ...(deadline.sharedWithSquad ? { sharedWithSquad: deadline.sharedWithSquad, squadId: deadline.sharedWithSquad } : {}),
    buddyNotified: false,
